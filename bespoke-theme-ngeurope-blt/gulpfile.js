...
gulp.task('browserify:lib', ['clean:browserify:lib', 'stylus'], function() {
  return gulp.src('lib/bespoke-theme-ngeurope-blt.js')
    .pipe(isDemo ? plumber() : through())
    .pipe(browserify({ transform: ['brfs'], standalone: 'bespoke.themes.ngeuropeBlt' }))
    ...
    .pipe(gulp.dest('dist'))
    .pipe(rename('bespoke-theme-ngeurope-blt.min.js'))
    .pipe(uglify())
    .pipe(header(template([
      '/*! <%= name %> v<%= version %> ',
